...
module.exports = {
  ...
  listRepos: (config, done) => {
    api.get(config, 'projects', (err, repos) => {
      ...
      done(null, repos.map(api.parseRepo).filter(repo => {
  ...
  getFile: (filename, ref, account, config, project, done) => {
    ...
    const uri = util.format('projects/%d/repository/blobs/%s?filepath=%s', repoId, branch, encodedFileName);
  ...
  getBranches: (account, config, project, done) => {
    ...
    api.get(account, uri, (err, branches) => {
      ...
      done(err, _.map(branches || [], branch => {
        return branch.name;
      }));
    });
  },
  ...
  routes: (app, context) => {
    app.post('/hook', (req, res) => {
      ...
      return api.createHooks(config, req.project.provider.repo_id, url, err => {
    ...
    app.delete('/hook', (req, res) => {
      ...
      return api.deleteHooks(config, req.project.provider.repo_id, url, (err, deleted) => {
        if(err) return res.status(500).send(err.message);
