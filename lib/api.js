...
function get(config, uri, done) {
  ...
  superagent
    ...
    .end((error, res) => {
      ...
      if(res.headers.link) {
        const linkHeader = parseLinkHeader(res.headers.link).last.page;
        const numberOfPages = linkHeader && linkHeader.last ? linkHeader.last.page : 1;
        let i = 1;
