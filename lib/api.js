...
function get(config, uri, done) {
  ...
  if (!config.api_url) {
    return done(
      new Error('API URL missing from GitLab provider configuration!')
    );
  }
  ...
  superagent
    ...
    .end((error, res) => {
      ...
      debug(
        `Response body received: ${util.inspect(res.body, false, 10, true)}`
      );
      if (res.text) {
        debug(
          `Response text received: ${util.inspect(res.text, false, 10, true)}`
        );
      }
      if (res.error) {
      ...
      if (res.headers.link) {
        ...
        const numberOfPages =
          linkHeader && linkHeader.last ? linkHeader.last.page : 1;
        let i = 1;
...
function createHooks(config, repo_id, url, callback) {
  ...
  superagent
    ...
    .set('PRIVATE-TOKEN', config.api_key)
    .set('User-Agent', 'StriderCD (http://stridercd.com)')
    .end((err, res) => {
      if (err) return callback(err);
      if (res.status === 200) {
        debug(
          'API request succeeded, but resource was not created. User possibly has insufficient access rights or GitLab API URL was not configured with correct prefix.'
        );
      }
...
function addDeployKey(config, repo_id, title, key, callback) {
  const endpoint_url = `${config.api_url}/projects/${encodeURIComponent(
    repo_id
  )}/deploy_keys`;
